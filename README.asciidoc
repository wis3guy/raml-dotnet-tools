...
** <<Cpass:[#]: Consuming an API>>
** <<Cpass:[#]: Calling an Authenticated API with OAuth>>
** <<Cpass:[#]: Consuming the HTTP Response>>
...
* Supported languages: Cpass:[#] (other languages indirectly)
...
+
. The Cpass:[#] classes nested beneath the parent RAML file contain the generated code to consume the Web API. At this point, the generated code is ready to be used.
...
The MoviesClient model object replicates the same structure as the RAML definition through available resources and actions. The methods in this object model are asynchronous and based on the Task Parallel Library (TPL), so they can executed with the new async  and await syntax in Cpass:[#] version 5.
...
Currently, Cpass:[#] is the only output language supported. This generated code can however simply be contained within a separate assembly, and the types exposed then consumed from any CLR language.
